...
(maxCurlSplits: Int) {
    ...
    init {
        ...
        mArrSelfShadowVertices = Array((mMaxCurlSplits + 2) * 2)
        mArrDropShadowVertices = Array((mMaxCurlSplits + 2) * 2)
        mArrTempShadowVertices = Array((mMaxCurlSplits + 2) * 2)
...
//        }
        ...
        run {
            ...
            val dist2 =
                sqrt((v0.mPosX - v2.mPosX) * (v0.mPosX - v2.mPosX) + (v0.mPosY - v2.mPosY) * (v0.mPosY - v2.mPosY))
            val dist3 =
                sqrt((v0.mPosX - v3.mPosX) * (v0.mPosX - v3.mPosX) + (v0.mPosY - v3.mPosY) * (v0.mPosY - v3.mPosY))
            if (dist2 > dist3) {
                lines[1][1] = 3
                lines[2][1] = 2
            }
        ...
        for (i in 0 until mArrScanLines.size()) {
            ...
            while (mArrOutputVertices.size() > 0) {
                ...
                if (DRAW_SHADOW && v.mPosZ > radius) {
                    ...
                    sv.mPosY = v.mPosY
                    sv.mPosZ = v.mPosZ
                    sv.mPenumbraX = (v.mPosZ - radius) / 6 * v.mPenumbraX
                    sv.mPenumbraY = (v.mPosZ - radius) / 6 * v.mPenumbraY
                    sv.mPenumbraColor = (v.mPosZ - radius) / (4 * radius)
        ...
        for (i in 0 until mArrDropShadowVertices!!.size()) {
            ...
            for (j in 0..3) {
                val color =
                    SHADOW_OUTER_COLOR[j] + (SHADOW_INNER_COLOR[j] - SHADOW_OUTER_COLOR[j]) * sv.mPenumbraColor
    ...
    private fun getIntersections(
        ...
        lineIndices: kotlin.Array<IntArray>,
        scanX: Double
    ): Array<Vertex> {
    ...
    fun onDrawFrame(gl: GL10) {
        ...
        val backStartIdx = 0.coerceAtLeast(mVerticesCountFront - 2)
